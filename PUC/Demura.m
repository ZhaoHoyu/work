...
Vdata_capture = NaN(r,c,3);
Vdata_capture(:,:,1) = Vdata_capture_R(gray)*ones(r,c);
...
%calculate offset
coff = (gray/255)^2.2;
V_interpolate=NaN(r,c,3);
V_interpolate(:,:,1)=interp1(gray_ideal_coefficient,Vdata(:,2,band),I(:,:,1)./lum_center(1)*coff);
...
gray_range=0:255;
gray_ideal_coefficient=(gray_range./255).^2.2;%ÁÁ¶È±ÈÀýÏµÊý
Vdata_ini=importdata('Vh.mat');
bandlist=1:13;
DBV=[2175,1600,1200,650,300,200,100,90,60,30,15,10,4];
Vdata=zeros(256,4,length(bandlist));
