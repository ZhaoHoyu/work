...
x1 = imread('C:\Users\F1_001\Desktop\4.png');
x2 = rgb2gray(x1);
% x2 = x2 * exp(3);% 进行傅里叶变换
x = double(x2);
...
% subplot(2,2,3), imshow(y_LPF, []), title('频域');
%% 高通滤波器
HPF = ones(size(y));        % 初始化低通滤波器
threshold_high = 500;         % 设置截至频率
[row, col] = size(y);       % 得到图像y的高度和宽度
for i = 1: row
    for j = 1: col
        % 计算频率
...
% subplot(2,2,4), imshow(y_HPF, []), title('频域');

%% 区间FFT
%% 高通滤波器
% IPF = ones(size(y));        % 初始化低通滤波器
% threshold_h = 10;         % 设置截至频率
% threshold_l = 120; 
% [row, col] = size(y);       % 得到图像y的高度和宽度
% for i = 1: row
%     for j = 1: col
...
%             % 将低通滤波器中高于截止频率的频率变为0
%             IPF(i,j) = 0;
%         else sqrt((i-row/2)^2+(j-col/2)^2) > threshold_l;
%             IPF(i,j) = 0;
%         end
%     end
% end
